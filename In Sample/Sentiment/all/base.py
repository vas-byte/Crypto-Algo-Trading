...
def run_backtest(ticker, cryptoname, reddit_folders, hours, macd_settings, MA_period, atr_period, obv_period, short, sentiment_positive, sentiment_negative, atr_mean):
    ...
    trades_df.to_csv(f'trades/{macd_settings[3]}_{MA_period}_{atr_period}_{obv_period}_{can_short}_{ticker}_{hours}_{atr_mean}_trades.csv', index=False)
    ...
    performance_df.to_csv(f'performance/{macd_settings[3]}_{MA_period}_{atr_period}_{obv_period}_{can_short}_{ticker}_{hours}_{atr_mean}_perf.csv', index=False)
...
macd_settings = [(12, 26, 9, 1), (8, 24, 9, 2), (5, 15, 9, 3)]
MA_periods = [20, 50]
atr = [10,14,20,50]
obv_periods = [10, 14, 20, 50]
allow_shorting = [True, False]
sentiment_thresholds_positive = [0.7,]
sentiment_thresholds_negative = [-0.5]
atr_means = [14, 20, 50]

# Multiprocess backtesting
if __name__ == "__main__":
    
    num_workers = 32  # Set your desired number of worker processes here
    with Pool(processes=num_workers) as pool:
        results = pool.starmap(run_backtest, [(ticker, crypto_names[ticker], reddit_folders[ticker], time, macd_setting, MA_period, atr_period, obv_period, short, sentiment_positive, sentiment_negative, atr_mean) for ticker in tickers for time in times for macd_setting in macd_settings for MA_period in MA_periods for atr_period in atr for obv_period in obv_periods for short in allow_shorting for sentiment_positive in sentiment_thresholds_positive for sentiment_negative in sentiment_thresholds_negative for atr_mean in atr_means])
