...
def sentiment_score(df):
    ...
    df = df.groupby(df['created_at'].dt.date)

    dates = []
    scores = []

    # Calculate weighted sentiment score
    for date, group in tqdm(df, desc="Calculating sentiment scores"):

        sentiments = date_sentiments = group['sentiment'].tolist()
        retweet_counts = group['retweet_count'].tolist()
        favorite_counts = group['favorite_count'].tolist()
        followers_counts = group['followers'].tolist()

        score = 0
        k = 0.000001

        for sentiment, rtc, favc, folc in zip(sentiments, retweet_counts, favorite_counts, followers_counts):
            sentiment_score = 1 if sentiment == "Positive" else -1 if sentiment == "Negative" else 0
            weight = favc * ((favc+1)/(folc+1)) * (rtc+1)
            score += sentiment_score * weight
        
        weighted = np.tanh(k * score)
        scores.append(weighted)
        dates.append(date)
    
    
    # Return dataframe with date and sentiment score
    sentiment_df = pd.DataFrame({'date': dates, 'sentiment_score': scores})
...
def generate_signal_and_size(row, bearish_threshold=-0.5, bullish_threshold=0.7):
    print(row)
    if row < bearish_threshold:
        return -1, row  # Bearish signal and position size
    elif row > bullish_threshold:
        return 1, row  # Bullish signal and position size
    else:
...
def get_pandas_df(tickers):
    ...
    for ticker in tickers:
        ...
        # Load text data
        text_df = pd.read_csv(f"../Data/{ticker}/twitter_deepseek.csv")

        if ticker == "BTC":
            # Load major events CSV
            major_events = pd.read_csv("BTC/bitcoin_events.csv")
            ...
            major_event_dates = major_events['date'].dt.date
            text_df = text_df[text_df['created_at'].dt.date.isin(major_event_dates)]

            print(text_df['created_at'].dt.date.value_counts())
            

        sentiment_df = sentiment_score(text_df)
      

        # Load XRP price data
        price = f"{ticker}/optimized_labeled_pred_mean_lag.csv" # @param {"type":"string","placeholder":"./raw/price.csv"}
        ...
        # Get the mean signal
        sentiment_df["signal"], sentiment_df["position_size"] = zip(*sentiment_df["sentiment_score"].apply(generate_signal_and_size))
        scaler = MinMaxScaler(feature_range=(0, 1))
        position_sizes = sentiment_df[['position_size']]  # Ensure it's a DataFrame
        sentiment_df['position_size'] = scaler.fit_transform(position_sizes)

        # Merge the means with the price data
        sentiment_df.index = pd.to_datetime(sentiment_df.index)
        price = price.merge(sentiment_df[['signal', 'position_size']], left_index=True, right_index=True, how='left')
