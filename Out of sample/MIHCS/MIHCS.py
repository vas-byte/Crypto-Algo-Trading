...
class MIHCS(bt.Strategy):
    ...
    def next(self):
        ...
        for crypto in self.datas:
            ...
            # Buy Signal
            can_buy = (self.ema_rsi[crypto][0] > 60) & (self.ema9[crypto][0] > self.ema20[crypto][0]) 

            # Sell Signal
            signal1 = (self.ema_rsi[crypto][0] > 60) & (self.ema9[crypto][0] <= self.ema20[crypto][0])
            signal2 = (self.ema_rsi[crypto][0] < 40) & (self.sar[crypto][0] > crypto.close[0]) & (self.sar[crypto][-1] < self.sar[crypto][0])
            signal3 = (self.ema_rsi[crypto][0] >= 40) & (self.ema_rsi[crypto][0] <= 60) & (self.rsi[crypto][0] > 55) & (self.rsi[crypto][-1] > self.rsi[crypto][0])
            can_sell = signal1 or signal2 or signal3
            ...
            # Sell
                ...
                if self.p.double_down and self.p.can_short:
                    # Place short order
                    order = self.sell(data=crypto, size=size, transmit=False)
