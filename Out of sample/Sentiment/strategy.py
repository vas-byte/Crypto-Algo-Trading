...
class SentimentIndicator(bt.Indicator):
    ...
    def __init__(self):
        self.twitter = pd.read_csv(f'../Data/{self.p.ticker}/twitter_deepseek.csv', parse_dates=True, date_parser=lambda x: pd.to_datetime(x, format="%a %b %d %H:%M:%S %z %Y"), index_col='created_at')
        
        # # Load Reddit data
        # self.reddit = pd.read_csv(f'../Data/Reddit/crypto_currency_news/sentiment_deepseek.csv', parse_dates=True)
        # self.reddit['created'] = pd.to_datetime(self.reddit['created'], unit='s')
        
        # # Load more Reddit data for cryptocurrency trading
        # cryptotrading = pd.read_csv(f'../Data/Reddit/cryptocurrencytrading/sentiment_deepseek.csv', parse_dates=True)
        # cryptotrading['created'] = pd.to_datetime(cryptotrading['created'], unit='s')
        # cryptomarkets = pd.read_csv(f'../Data/Reddit/cryptomarkets/sentiment_deepseek.csv', parse_dates=True)
        ...
        #     subreddit_data = pd.read_csv(f'../Data/Reddit/{folder}/sentiment_deepseek.csv', parse_dates=True)
        ...
        # self.news = pd.read_csv(f'../Data/news_deepseek.csv', parse_dates=True)
...
class Sentiment(bt.Strategy):
    params = (
        ...
        ('stop_loss', 0.05),
        ('can_short', True),
        ('cryptoname', {'DOGEUSDT':'dogecoin'}),
        ('k', 0.000001),
        ('macd_fast', 12),
        ('macd_slow', 26),
        ('macd_signal', 9),
        ('atr_period', 14),
        # ('reddit_folder', ['dogecoin']),
        ('obv_period', 14),
    ...
    def start(self):
        for data in self.datas:
            ...
            self.sentiment[data] = SentimentIndicator(data, ticker=name, crypto=self.p.cryptoname[raw_name], k=self.p.k)
    ...
    def next(self):
        ...
        for crypto in self.datas:
            ...
            elif self.getposition(crypto).size > 0 and (not long) and (macd_short and obv_short):
                self.cancel_limit_orders(crypto=crypto)
                self.close(data=crypto)

            # Close the short position (exit signal)
            elif self.getposition(crypto).size < 0 and (not short) and (macd_long and obv_long):
                self.cancel_limit_orders(crypto=crypto)
                self.close(data=crypto)
