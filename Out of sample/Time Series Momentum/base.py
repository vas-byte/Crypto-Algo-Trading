...
class MarketCapCSV(bt.feeds.GenericCSVData):
    ...
    params = (
        ('dtformat', '%Y-%m-%d'),  # Adjust the date format if necessary
        ('datetime', 0),
        ('open', 1),
        ('high', 2),
        ('low', 3),
...
cerebro.addanalyzer(btanalyzers.SharpeRatio, _name='sharpe', timeframe=bt.TimeFrame.Days, factor=365, riskfreerate=0.0518, annualize=True)
cerebro.addanalyzer(btanalyzers.TradeAnalyzer, _name='trades')
cerebro.addanalyzer(btanalyzers.Returns, _name='returns')
cerebro.addanalyzer(bt.analyzers.DrawDown, _name="drawdown")

cerebro.addstrategy(TimeSeries, can_short=True)
